...
import time
pipe=Pipe()
folder_path=r'C:\ProgramData\lcm666'
...
def udp_receive(pipe0):
    ...
    while True:
        ...
        if data:
            i = int.from_bytes(data[0:2], byteorder='big')  # 一张中的第几组数据 i
            j = int.from_bytes(data[2:4], byteorder='big')  # 一张中的共有j组 j
            k = int.from_bytes(data[4:6], byteorder='big')  # 校验和 k
            l = int.from_bytes(data[6:8], byteorder='big')  # l
            buffer_index = l % 5
